...
//     //    Dado o seguinte array de pokémons, realize as operações pedidas nos itens abaixo:
   ...
   // let arrayFinal; 
   // let i; 
   //  let resultado = pokemons.filter((item) =>  item.tipo !== pokemons[0].tipo);
   //    arrayFinal = [pokemons[0]];

   //  resultado = resultado.filter((item) => item.tipo !== pokemons[1].tipo )
   // arrayFinal = [ ...arrayFinal, pokemons[1]];

 
   //  resultado = resultado.filter((item) => item.tipo !== pokemons[2].tipo);
   // arrayFinal = [ ...arrayFinal, pokemons[2]];
